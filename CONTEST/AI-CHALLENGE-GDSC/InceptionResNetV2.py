...
training_images = np.array([plt.imread(f'../input/gdscaidataset/train/train/{i + 1}.png') for i in range(0, 50000)])
testing_images  = np.array([plt.imread(f'../input/gdscaidataset/test/test/{i + 1}.png')   for i in range(0, 20000)])

# Import RestNet50 or InceptionResNetV2 to train
#conv_base = ResNet50(weights='imagenet', include_top=False, input_shape=(224, 224, 3))
conv_base = tf.keras.applications.inception_resnet_v2.InceptionResNetV2(weights='imagenet', include_top=False, input_shape=(224, 224, 3))
...
batch_size = 32
epochs=10
opt_rms = tf.keras.optimizers.SGD(learning_rate=0.001, momentum=0.9,decay=1e-6, nesterov=False)
model.compile(loss='binary_crossentropy',
        optimizer=opt_rms,
        metrics=['accuracy'])
# steps_per_epoch=training_images.shape[0]// batch_size
model.fit(datagen.flow(training_images, training_labels, batch_size=batch_size),epochs=2*epochs,verbose=1)

opt_rms = tf.keras.optimizers.SGD(learning_rate=0.0005, momentum=0.9,decay=1e-6, nesterov=False)
model.compile(loss='binary_crossentropy',
        optimizer=opt_rms,
        metrics=['accuracy'])
model.fit(datagen.flow(training_images, training_labels, batch_size=batch_size),epochs=epochs,verbose=1)

opt_rms = tf.keras.optimizers.SGD(learning_rate=0.0003, momentum=0.9,decay=1e-6, nesterov=False)
model.compile(loss='binary_crossentropy',
        optimizer=opt_rms,
        metrics=['accuracy'])
model.fit(datagen.flow(training_images, training_labels, batch_size=batch_size),epochs=epochs,verbose=1)

opt_rms = tf.keras.optimizers.SGD(learning_rate=0.0008, momentum=0.9,decay=1e-6, nesterov=False)
model.compile(loss='binary_crossentropy',
        optimizer=opt_rms,
        metrics=['accuracy'])
model.fit(datagen.flow(training_images, training_labels, batch_size=batch_size),epochs=epochs,verbose=1)
